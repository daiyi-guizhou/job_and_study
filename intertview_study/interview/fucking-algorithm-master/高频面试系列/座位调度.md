...
有趣且具有一定技巧性。这种题目并不像动态规划这类算法拼智商，而是看你对常用数据结构的理解和写代码的水平，个人认为值得重视和学习。
...
"""
作者：LeetCode
链接：https://leetcode-cn.com/problems/exam-room/solution/kao-chang-jiu-zuo-by-leetcode/
来源：力扣（LeetCode）
...
class ExamRoom:
    ...
    def seat(self) -> int:
        if not self.student: 
            ...
            return 0
        # 这句话很牛批! 细品 代表了第一段的distance, 无论第一位同学是不是0
        distance, min_seat = self.student[0], 0
        for i in range(1, len(self.student)):
            tmp = (self.student[i]-self.student[i-1]) // 2
            if tmp > distance:
                distance = tmp
                min_seat = (self.student[i]+self.student[i-1])//2
        if self.n-1 - self.student[-1] > distance: # 两个边界点和其他点的的距离只需考虑一边, 所以不用除2!
            min_seat = self.n-1
        insort(self.student, min_seat)
        # print(self.student)
        return min_seat  
...
"""
作者：yaosw
链接：https://leetcode-cn.com/problems/exam-room/solution/bisec-by-yaosw/
来源：力扣（LeetCode）
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
"""
